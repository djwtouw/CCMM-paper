%% ---------------------------------------------------------
%% This is the implementation of fast AMA 
%% Reference:
...
%% ------------------------------------------------------------
function [Z, X, info] = fast_AMA(Ainput,b,dim,weightVec,options,Z0)
   maxiter = 20000; 
   t_start = clock();
   rho = eigs(Ainput.ATAmat,1);
   mu = 1/(rho+1);
   stoptol = 1e-6;
   rng('default');
   ...
   while (info.dualgap > stoptol)
       ...
       proj_input = Z_tilde - mu*gvec;
       Z = proj_l2(proj_input, weightVec);
       alpha = (1 + sqrt(1+4*alpha_old*alpha_old))/2.0;
       Z_tilde = Z + (alpha_old/alpha)*(Z - Zold);
       Zold = Z;
       alpha_old = alpha;
       %% compute duality gap
       ATZ = ATmap(Z);
